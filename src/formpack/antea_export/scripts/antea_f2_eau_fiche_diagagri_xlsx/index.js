...
//node index_AG.js -t Template_Agri_avec_T_Bilan_et_Siege.xlsx -d Projet_Anais_10agri.json
...
//console.log(res)
// 01/03/2021 : ajout des exports complémentaires pour le siège (100 colonnes dans onglet T_SIEGE)
var res_siege = utils.formatting_siege(jsonContent)
...
// 09/02/2021 : ajout d'un group repeat => GP9 (stockage engrais + cuve Azote liquide)
var res_GP9 = utils.formatting_GP9(jsonContent)
// console.log(res_GP8)
/*
nb = 0
...
fs.readFile(option.templateFilePath, function(err, xlsx_data) {
	...
	t.substitute("T_SIEGE", res_siege);
	t.substitute("Cultures_N", res_GP1);
	t.substitute("Cultures_Nm1", res_GP2);
	t.substitute("Cultures_Nm2", res_GP3);
	t.substitute("Chargement_Ha", res_GP4);
	t.substitute("Fiche_parcelle_N", res_GP5);
	t.substitute("Fiche_parcelle_Nm1", res_GP6);
	t.substitute("Fiche_parcelle_Nm2", res_GP7);
	t.substitute("Diag_Siege_Expl_Stock", res_GP8);
	// 09/02/2021 : ajout d'un group repeat => GP9 (stockage engrais + cuve Azote liquide)
	t.substitute("Stockage_Engrais", res_GP9);

...
class AnteaExport(AnteaExportXSLX):
    ...
    def export_carto(self, json_path):
        ...
        for element in json:
            try:
                feature_point = self.create_point_from_json(element, "INFO_GENERAL-_INFO_GENERAL-PR_COORDS")
                webMapPoint = utils_point.WebMapPoint(feature_point)
                self.export_webmap(webMapPoint, exportCarto, "pr_coord_{}.png".format(element["_index"]))
    ...
    def compute(self):
        self.tmp_folder = self.standard_init(self.exportPath)
